...
import { useOfferStore } from '../state/useOfferStore';
...
export function PaywallContent({
  ...
  useEffect(() => {
    ...
    analytics.track(AnalyticsEvents.PAYWALL_VARIANT_VIEWED, {
  ...
  const scheduleReminderIfEligible = async () => {
    if (!isFeatureEnabled('trialReminder')) return;
  ...
  const handlePurchase = async (pkg: SubscriptionPackage) => {
    ...
    try {
      analytics.track(AnalyticsEvents.PURCHASE_STARTED, {
      ...
      if (error.message === 'PURCHASE_CANCELLED') {
        analytics.track(AnalyticsEvents.PURCHASE_CANCELLED, {
          package: pkg.identifier,
          variant: offerState.offerType,
          influencer_name: offerState.influencerName,
        });
  ...
  const getPackageTitle = (pkg: SubscriptionPackage): string => {
  ...
  const getPackageSubtitle = (pkg: SubscriptionPackage): string => {
    const id = pkg.identifier.toLowerCase();
  ...
  const getPackageBadge = (pkg: SubscriptionPackage): string | null => {
    const id = pkg.identifier.toLowerCase();
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {/* Benefits */}
        {isFeatureEnabled('paywallBenefits') && (
        ...
        {/* Trust Signals */}
        {isFeatureEnabled('paywallTrustSignals') && variant !== 'gift' && (
        ...
        {__DEV__ && !isFeatureEnabled('subscriptionRequired') && (
